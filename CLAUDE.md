...
| `DOUBLE_CLICK_EVENT` | メニュー（グラス側）を開く。一番上が「非表示にする」、その下に UP/DOWN の移動単位（1画面ごと／練習番号／ページ番号／見出し2／見出し3）の5択 |

**非表示モード**（セッション5、メニューの「非表示にする」）：ボディ・フッター両コンテナをブランクにし、ボディの中央行に「・」1文字だけを表示する（`performer.ts` の `buildHiddenBodyText()`）。中身を完全な空白にしなかった理由は、グラス自体のハードウェア側「サイレントモード」（このアプリからは検知も制御もできない）が同じく画面を暗くするため、何も表示しない状態だと「アプリが意図して非表示にしている」のか「サイレントモードで単に何も来ていない」のか区別が付かないから——ユーザーからの明示的な要望。非表示中は CLICK 以外の入力（UP/DOWN・ダブルタップ）はすべて無視し、CLICK で直前の表示（節・表示ウィンドウとも）にそのまま復帰する。`blockIndex`/`lineOffset` は非表示中も変更しないので、`exitHidden()` はただの `render()` 呼び出しで済む。メニュー中に「非表示にする」を選んでも `picker` を `null` に戻してから `enterHidden()` を呼ぶので、非表示状態とメニュー表示中状態が同時に立つことはない。
...
**UP/DOWN の移動単位の選び方**（1画面ごと／練習番号／ページ番号／見出し2／見出し3の5択）：スマホ側 `Settings.upDownMode`（`src/settings.ts`）は演奏開始時の**既定値**でしかない。実際にどの単位で移動するかは `performer.ts` の `startPerformance` 内のローカル変数 `upDownMode` が持ち、ダブルタップで開く画面（`openModePicker`/`renderPicker`/`movePicker`/`confirmPicker`）で選び直せる。この選択は Settings には書き戻さない（次回の演奏はまたスマホ側の既定値から始まる）。この画面は SDK 0.0.14 にある `ListContainerProperty`/`MenuContainerProperty`（選択状態を OS 側が持ってくれる本来の「リスト/メニュー」コンテナ）ではなく、既存の body/footer `TextContainer` に選択肢を素朴に描画し（選択行を `"> "` で示す）、CLICK/UP/DOWN を自前でハンドリングする実装にしている——理由は、List/Menu コンテナの選択イベントのスキーマ（`List_ItemEvent`/`MenuItemClickEvent`）をこのシミュレータに対して一度も検証しておらず、確実に動くと分かっている `textContainerUpgrade` の経路だけで実現できるため。グラス側曲選択（下記「コーディング方針」）で本格的にリスト/メニューコンテナが要る場合は、その時に改めて型定義とシミュレータ実機動作を確認すること。

**「1画面ごと」モード**（`UpDownMode` の `'window'`、型は `src/types.ts`）：他の4モードが `findBoundary`（値が変わる境界まで直進）でジャンプするのに対し、これだけは境界探索をせず `advanceWindow()` で CLICK と同じ表示ウィンドウ送り（`maxLines - 1` のオーバーラップ幅）を前後どちらの方向にも行う——「UP/DOWNで単にページ送り・戻しをしたい」という要望（練習番号・ページ番号マーカーの有無に関係なく機械的に画面単位で送れる）に応えたもの。ラベルは意図的に「ページ」ではなく「画面」を使っている（`ページ番号ごと`モードと紛らわしくなるのを避けるため——あちらは本文中の `[X]` マーカー境界へのジャンプで、これは単なる表示ウィンドウの送り戻し）。`jumpBoundary()` が `mode === 'window'` を先頭で分岐して `valueAt`/`findBoundary` に渡さない（`valueAt`/`findBoundary` の `mode` 引数の型は `Exclude<UpDownMode, 'window'>` — `'window'` はそもそも渡らない設計）。
...
**電話画面側 UI の見た目は `GET /api/screenshot/webview` で確認可能**（グラス側だけでなくこちらもスクリーンショットが撮れる）。ボタンクリックやフォーム入力など webview 内の**操作**は自動化できないが、**見た目**は検証済み：曲一覧の空状態・同期済み/同期エラー状態の行、設定パネル（輝度スライダー・チェックボックス・UP/DOWNモード既定値の select——ただし「1画面ごと」追加前の4択時点でのスクリーンショット確認）、演奏中画面（新しいリング操作ヘルプ文言・「終了してライブラリに戻る」ボタン）を、`index.html` に一時的な `localStorage` シード用 `<script>` を挿入する方法、および `main.ts` の初期 `view` を一時的に書き換える方法（どちらも確認後に元に戻す）でレンダリングさせて確認した。

### セッション4：UP/DOWN「1画面ごと」モード追加

検証済み（`test-harness.ts`/`test-harness.html` を一時的に作成し `parseSong`/`startPerformance` を直接叩いて確認、確認後に削除。ブロック長20行＋3行の2ブロック構成の曲で検証）：
- `advanceWindow()` の前進：ブロック内で `(1-8)(8-15)(13-20)` と1行オーバーラップ・末尾クランプで進み、ブロックが尽きたら次ブロック先頭（表示ウィンドウのスライドアルゴリズムと同じ式で前進していること）、最終ブロックが画面に収まっている状態でさらに DOWN しても何も起きないこと（曲末での無反応）
- `advanceWindow()` の後退：ブロック先頭で UP すると前ブロックの**最終ウィンドウ**（クランプ済みの位置）に戻ること、ブロック内では前進と対称に7行ずつ戻ること、曲頭で UP しても何も起きないこと
- ダブルタップの移動単位選択画面に「1画面ごと（単純に送り戻し）」が5択中の先頭に表示され、カーソル移動・CLICK確定が機能すること、確定後に他モード（`practice`）との相互切り替えで それぞれ正しい移動方式（`window` は単純送り、`practice` は境界ジャンプ）に切り替わること
- `tsc --noEmit` / `vite build`

未検証（今後の課題）：
- スマホ側設定パネルの select に「1画面ごと」の項目が実際に描画される見た目（`GET /api/screenshot/webview`）——実ライブラリ（同期済みの実曲データ）がこの開発環境の localStorage に残っており、誤って壊さないよう settings 以外のキーには触れなかった結果、ページが長くスクロールが必要になり select 部分まで到達するスクリーンショットが撮れなかった。コード自体は既存4択と全く同じパターン（`<option>` 追加のみ）なので実装リスクは低いと判断し、`tsc`/`vite build` の型チェックのみで済ませた。次にこの画面周りを触るセッションは、本番相当ではない空のライブラリ状態（またはページを縮めるための一時的なDOM操作）で改めてこの select を screenshot 確認すること。

### セッション5：ダブルタップメニューに「非表示にする」を追加

検証済み（`test-harness.ts`/`test-harness.html` を一時的に作成し `parseSong`/`startPerformance` を直接叩いて確認、確認後に削除。1ブロック3行の簡易曲で検証）：
- ダブルタップで開くメニューの一番上に「非表示にする」が表示され、他5項目（UP/DOWN単位）が続くこと。既存の挙動通りカーソルの初期位置は現在の UP/DOWN モードの項目
- 「非表示にする」を選んで CLICK 確定すると、ボディ・フッターとも空白になり、ボディ中央行に「・」1文字だけが表示されること（スクリーンショットで確認）
- 非表示中に UP／DOWN／ダブルタップを送っても無反応（画面が変化しない）こと
- 非表示中に CLICK すると、非表示にする直前と全く同じ表示（本文3行＋フッターの練習番号／曲名／ページ番号）に復帰すること
- 復帰後も通常の CLICK（次データへ）が正常に動作し続けること（内部状態が壊れていないこと）
- `tsc --noEmit` / `vite build`

未検証（今後の課題）：
- 非表示モードの「・」表示を、既存の「実際に見つかった不具合」4番（フッター右端の縦棒アーティファクト）と合わせて実機で見たときの見え方——シミュレータのスクリーンショットでは「・」はやや左寄り・やや上寄りに出ており（`buildHiddenBodyText()` の行送り・spx 計算の丸めによるもの）、厳密な中心ではない。実用上問題ないと判断したが、実機で気になるレベルかは未確認。
- スマホ側設定パネル・演奏中画面に「非表示にする」を反映すべき見た目要素は無い（グラス側のみの機能のため）ので webview 側スクリーンショットは対象外。

### セッション6：スマホ設定「ダブルタップで直接非表示にする」を追加

`Settings.doubleClickHidesDirectly`（`src/types.ts`/`src/settings.ts`、既定 `false`）。ON のとき、グラス側 DOUBLE_CLICK はメニューを経由せず `enterHidden()` を直接呼ぶ（`performer.ts` のイベントハンドラで `openModePicker()` と分岐）。この設定は演奏開始時に一度きり参照されるのではなく、双方向スワイプの既定値と違って演奏中に変わり得ないので `settings.doubleClickHidesDirectly` をイベントハンドラで直接読んでいる（`upDownMode` のようにローカル変数へコピーしていない）。電話画面の演奏中ヘルプ文言（`ui.ts` の `renderPerforming`）もこの設定値で出し分けている（ON: 「ダブルタップ: 非表示にする」／OFF: 「ダブルタップ: メニューを開く（非表示 / 移動単位の選択）」）ため `renderPerforming` のシグネチャに `settings` を追加した（`main.ts` の呼び出し箇所も更新）。

検証済み（`test-harness.ts`/`test-harness.html` を一時的に作成し、`doubleClickHidesDirectly: true` の `Settings` で `startPerformance` を直接叩いて確認、確認後に削除）：
- ON の状態で DOUBLE_CLICK を送るとメニューを経由せず直接「・」表示（非表示モード）になること
- 非表示中に2回目の DOUBLE_CLICK を送っても無反応（メニューも開かず、非表示のまま）なこと——非表示中は CLICK 以外を無視する既存ロジックがそのまま効いている
- CLICK で非表示にする直前の表示に正しく復帰すること
- `tsc --noEmit` / `vite build`

未検証（今後の課題）：
- スマホ側設定パネルに追加したチェックボックス「ダブルタップで直接非表示にする」の実際の描画見た目——実ライブラリ（同期済みの実曲データ4件）が縦に長く、`GET /api/screenshot/webview` はビューポート分しか写らず設定パネルまでスクロールが必要。Tauri のネイティブウィンドウをスクロールさせる自動化手段がこの環境に無いため（前回セッションが「1画面ごと」の select 確認時に直面したのと同じ制約）、他3つの既存チェックボックスと全く同じマークアップパターンであることを目視コードレビューで確認した上で、型チェックのみで済ませた。次にこの設定パネル周りを触るセッションは、空のライブラリ状態（またはウィンドウ/ビューポートを広げる手段）でこのチェックボックスをスクリーンショット確認すること。
- ON 時、通常のダブルタップメニュー（UP/DOWN単位選択）に到達する手段が演奏中に無くなる（OFFに戻すには一旦演奏を終了してスマホ側設定を変更する必要がある）という UX 上のトレードオフはユーザーの要望通りの仕様として実装しており、変更の要否は未確認。
...
- **ハードコードのプリセット曲が1件ある**（`src/library.ts` の `seedDefaultSongs()`）：タイトル「シュミット」、URL はこのリポジトリ自身の `prompts/202609-Schmitt-Seven.md` の raw URL。ユーザーから最初に指定されたのは `github.com/.../blob/...` の閲覧用URLだったが、これは HTML ページを返し CORS ヘッダも無いため `raw.githubusercontent.com/...` に変換して使っている（「一次情報源の優先順位」節と同じ理由——このプロジェクトの fetch は raw 系ホストのみ動作する設計）。`localStorage`（`chorus-prompter:defaultSongsSeeded` フラグ）で「アプリの初回起動時に一度だけ」追加する方式にしており、「ライブラリが空なら追加」ではない——後でユーザーがこのプリセットを削除しても、ライブラリが再び空になった時に復活しないようにするため。追加時は手動の「曲を追加」と同じく自動で同期も行う（`main.ts` が `doSync` を呼ぶ）。シミュレータで実際にこの自動シード→自動同期（84節）→再起動しても重複追加されないこと、を確認済み。
