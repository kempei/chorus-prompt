...
| `SCROLL_TOP_EVENT`（UP） | 前の練習番号／ページ番号の境界へジャンプ（どちらを使うかは電話側設定 `Settings.upDownMode`） |
| `SCROLL_BOTTOM_EVENT`（DOWN） | 次の練習番号／ページ番号の境界へジャンプ |
| `DOUBLE_CLICK_EVENT` | 直前の UP/DOWN と同じ方向へ10境界分ジャンプ（このセッションで一度も UP/DOWN していなければ何もしない） |
...
### 検証済み・未検証の整理（`spec.md` 対応セッション終了時点）
...
- DOUBLE_CLICK による「直前スクロール方向へ10境界ジャンプ」、および曲頭・曲末でのクランプ（それ以上進まない）
...
- **グラス側に終了操作は無い**。DOUBLE_CLICK は spec.md で「直前スクロール方向へ10単位移動」に再割当てされ、長押しは実機ではシステムメニューに奪われる可能性が高くアプリに機能を割り当てられないとユーザーが判断したため、どちらも終了操作に使わない。電話画面の「終了してライブラリに戻る」ボタンが唯一の終了手段であり、`performer.ts` の `stop()` はここから直接 `bridge.shutDownPageContainer(1)` を呼ぶ（旧実装はダブルタップ経由でのみこれを呼んでいた）。
...
npm run pack      # evenhub pack app.json dist（配布用パッケージング）
```
